...
HASHTAG_SPEC = 'tagdaynheem'
USER_SPEC = 'userdaynheem'
URL_SPEC = 'urldaynheem'
EMOJI_SPEC = 'emojidaynheem'
...
####################### ADD SPECIAL TOKEN ##############################
tokenizer.add_tokens([HASHTAG_SPEC, USER_SPEC, URL_SPEC, EMOJI_SPEC])
weights = model.bert.embeddings.word_embeddings.weight.data
new_weights = torch.cat((weights, weights[:4]), 0)
new_emb = nn.Embedding.from_pretrained(new_weights, padding_idx=tokenizer.pad_token_id, freeze=False)
...
def preprocess_data(examples):
    ...
    model_inputs = tokenizer(text_inps, max_length=max_input_length, truncation=True)
